...
fn main() {
    ...
    // uncomment next line and comment all the concurrent code to run on a single thread
    // render(&mut pixels, bounds, upper_left, lower_right);
    
    // CODE TO RENDER CONCURRENTLY BEGINS
    let threads = 8;
    ...
    crossbeam::scope(
        |spawner| {
            for (i, band) in bands.into_iter().enumerate() {
                let (top, height) = (rows_per_band * i, band.len() / bounds.0);
    ...
    // CODE TO RENDER CONCURRENTLY ENDS
